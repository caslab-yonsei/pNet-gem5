...
void e1000_check_options(struct e1000_adapter *adapter)
{
	...
	//////printk(KERN_ALERT "NEPU e1000_check_options was called\n");
	...
	{ /* Transmit Interrupt Delay */
		...
		if (num_TxIntDelay > bd) {
			adapter->tx_int_delay[i] = TxIntDelay[bd];
			e1000_validate_option(&adapter->tx_int_delay[i], &opt,
			                      adapter);
		} else {
			adapter->tx_int_delay[i] = opt.def;
		}}
	}
	{ /* Transmit Absolute Interrupt Delay */
		...
		if (num_TxAbsIntDelay > bd) {
			adapter->tx_abs_int_delay[i] = TxAbsIntDelay[bd];
			e1000_validate_option(&adapter->tx_abs_int_delay[i], &opt,
					      adapter);
		} else {
			adapter->tx_abs_int_delay[i] = opt.def;
		}}
	}
	{ /* Receive Interrupt Delay */
		...
		if (num_RxIntDelay > bd) {
			adapter->rx_int_delay[i] = RxIntDelay[bd];
			e1000_validate_option(&adapter->rx_int_delay[i], &opt,
					      adapter);
		} else {
			adapter->rx_int_delay[i] = opt.def;
		}}
	}
	{ /* Receive Absolute Interrupt Delay */
		...
		if (num_RxAbsIntDelay > bd) {
			adapter->rx_abs_int_delay[i] = RxAbsIntDelay[bd];
			e1000_validate_option(&adapter->rx_abs_int_delay[i], &opt,
					      adapter);
		} else {
			adapter->rx_abs_int_delay[i] = opt.def;
		}}
	}
	{ /* Interrupt Throttling Rate */
		...
		if (num_InterruptThrottleRate > bd) {
			...
			adapter->itr[i] = InterruptThrottleRate[bd];
			switch (adapter->itr[i]) {
			case 0:
			...
			case 1:
				e_dev_info("%s set to dynamic mode\n",
					   opt.name);
				adapter->itr_setting[i] = adapter->itr[i];
				adapter->itr[i] = 20000;
				break;
			case 3:
			...
			case 4:
				...
				break;
			default:
				e1000_validate_option(&adapter->itr[i], &opt,
						      adapter);
				 ...
				 * used as control
				 */
				adapter->itr_setting[i] = adapter->itr[i] & ~3;
				break;
			}
		} else {
			adapter->itr_setting[i] = opt.def;
			adapter->itr[i] = 20000;
		}
		}
	}
